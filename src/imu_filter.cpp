...
void ImuMagUpdateTask(void *arg)
{
  uint32_t last_ticks = xTaskGetTickCount();
  ...
  Serial.println("Initialize BMM150");

  xSemaphoreTake(i2c_lock, portMAX_DELAY);

  if (bmm150.initialize() != BMM150_OK)
  {
    Serial.println("BMM150 cannot init!");
  }
  else
  {
  ...
  bmm150.set_op_mode(BMM150_NORMAL_MODE);
  bmm150.set_presetmode(BMM150_PRESETMODE_HIGHACCURACY);
  xSemaphoreGive(i2c_lock);
